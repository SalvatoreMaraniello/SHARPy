...
subroutine opt_main( NumElems, NumNodes,                                       & ! Problem SetUp
    ...
    ! managed in the python wrapper. They are set to zero and passed to the
    ! fwdsolver to allow consistency with old code.
    ...
    ! ----------------------------------------------------------- To be removed?
    character(len=4), intent(inout)  :: IN_TestCase       ! unused.


    ! -------------------------------------------------------------------- Input

    ! Design Variables Shared
    ! Originally set through input_setup.
    ...
    ! passed as an input to check that the preallocated variables in the python
    ...
    real(8)  :: PhiNodes(NumNodes)              ! Initial Twist angle (opt). This
                                                ! variable overwrites PhiNodes in the input setup

    ! Problem Setup Shared
    ! variables that appear as shared in the input module.
    integer,          intent(inout)  :: IN_NumNodesElem   ! Number of nodes on each element.
    character(len=4), intent(inout)  :: IN_ElemType       ! ='STRN','DISP'
    character(len=2), intent(inout)  :: IN_BConds         ! 'CF','CC','CS','SC','SS','SF'


    ! ------------------------------------------------------------------- Output

    ! General
    real(8), intent(inout) :: DensityVector (NumElems) ! Linear Density of each element of the beam (for cost evaluation).
    real(8), intent(inout) :: LengthVector (NumElems)  ! Length of each element of the beam (for cost evaluation).
    ...
    integer,  allocatable:: BoundConds(:)  ! =0: no BC; =1: clamped nodes; =-1: free node; =2: spherical (sol. 102,912,932)
 ...
 ! ---------------------------------------------------------------- pack Options
    ...
    select case (Options%Solution)
        case (102,112,142,202,212,302,312,322,900,902,910,912,922,932,952)
            IN_ElemType= 'DISP'
        case default
            STOP '[opt_routine, opt_main] Error: Wrong solution code (51707)'
    end select
    ...
    if ( (solmode == 'OPT') .or. (solmode == 'SNS') ) then
        ...
        if (solmode == 'SNS') then  ! (only needed for the allocation)
            NOPTMAX=0
        end if
